...
def train():
    ...
    model = MultiHeCo(args.hidden_dim, feats_dim_list, args.feat_drop, args.attn_drop,
    ...
    for epoch in range(args.nb_epochs):
        ...
        if loss < best:
            ...
            best_t = epoch
            cnt_wait = 0
            torch.save(model.state_dict(), 'MultiHeCo_'+own_str+'.pkl')
        else:
            cnt_wait += 1
    ...
    model.load_state_dict(torch.load('MultiHeCo_'+own_str+'.pkl'))
    model.eval()
    os.remove('MultiHeCo_'+own_str+'.pkl')
